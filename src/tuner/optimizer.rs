...
pub fn optimize_k(dataset_path: &str, max_number_of_samples: usize) -> Result<f32, io::Error> {
  ...
  Chart::new(100, 40, 0.002, 0.0027).lineplot(&Shape::Lines(&errors)).display();

  println!("Winnig Probability with {best_k}");
  Chart::new(100, 40, -20.0, 20.0)
    .lineplot(&Shape::Continuous(Box::new(|x| sigmoid(best_k, x * 100.0))))
    .display();
