...
#[inline(always)]
pub fn get_bishop_square_to_square_ray(king_square: Square, bishop_square: Square) -> BitBoard {
  SQUARE_TO_SQUARE_RAY.bishop_rays[king_square as usize][bishop_square as usize]
}

#[inline(always)]
pub fn get_rook_square_to_square_ray(king_square: Square, rook_square: Square) -> BitBoard {
  SQUARE_TO_SQUARE_RAY.rook_rays[king_square as usize][rook_square as usize]
}

#[inline(always)]
pub fn get_pawns_attacks(side: PieceColor, square: Square) -> BitBoard {
  LOOK_UP_TABLE.pawn_attacks[side][square as usize]
}

#[inline(always)]
pub fn get_knight_attacks(square: Square) -> BitBoard {
  LOOK_UP_TABLE.knight_attacks[square as usize]
}

#[inline(always)]
pub fn get_bishop_attacks(board: BitBoard, square: Square) -> BitBoard {
  let occupancy = board & LOOK_UP_TABLE.bishop_blockers_masks[square as usize];
  LOOK_UP_TABLE.bishop_attacks[square as usize][(occupancy.wrapping_mul(LOOK_UP_TABLE.bishop_magics[square as usize]) >> 55) as usize]
}

#[inline(always)]
pub fn get_rook_attacks(board: BitBoard, square: Square) -> BitBoard {
  let occupancy = board & LOOK_UP_TABLE.rook_blockers_masks[square as usize];
  LOOK_UP_TABLE.rook_attacks[square as usize][(occupancy.wrapping_mul(LOOK_UP_TABLE.rook_magics[square as usize]) >> 52) as usize]
}

#[inline(always)]
pub fn get_king_attacks(square: Square) -> BitBoard {
  LOOK_UP_TABLE.king_attacks[square as usize]
}

pub struct SquareToSquareRay {
  pub bishop_rays: [[BitBoard; 64]; 64],
  pub rook_rays: [[BitBoard; 64]; 64],
}

impl SquareToSquareRay {
  pub fn init() -> SquareToSquareRay {
    let mut bishop_rays: [[BitBoard; 64]; 64] = [[0; 64]; 64];
    let mut rook_rays: [[BitBoard; 64]; 64] = [[0; 64]; 64];

    for king_square in 0..64 {
      for bishop_square in 0..64 {
        let king_mask = get_bishop_attacks(1u64 << bishop_square, king_square);
        let bishop_mask = get_bishop_attacks(1u64 << king_square, bishop_square);
        if (king_mask & (bishop_mask | (1u64 << bishop_square))) != 0 {
          bishop_rays[king_square as usize][bishop_square as usize] = king_mask & (bishop_mask | (1u64 << bishop_square));
        }
      }
    }

    for king_square in 0..64 {
      for rook_square in 0..64 {
        let king_mask = get_rook_attacks(1u64 << rook_square, king_square);
        let rook_mask = get_rook_attacks(1u64 << king_square, rook_square);
        if (king_mask & (rook_mask | (1u64 << rook_square))) != 0 {
          rook_rays[king_square as usize][rook_square as usize] = king_mask & (rook_mask | (1u64 << rook_square));
        }
      }
    }

    SquareToSquareRay { bishop_rays, rook_rays }
  }
}

struct LookUpTable {
  pub pawn_attacks: ByColor<[BitBoard; 64]>,
  pub knight_attacks: [BitBoard; 64],
  pub king_attacks: [BitBoard; 64],
  pub bishop_blockers_masks: [BitBoard; 64],
  pub bishop_magics: [BitBoard; 64],
  pub bishop_attacks: Box<[[BitBoard; 512]; 64]>,
  pub rook_blockers_masks: [BitBoard; 64],
  pub rook_magics: [BitBoard; 64],
  pub rook_attacks: Box<[[BitBoard; 4096]; 64]>,
}

impl LookUpTable {
  pub fn init() -> LookUpTable {
    let rook_magics_and_attacks = LookUpTable::generate_rook_attacks();
    let bishop_magics_and_attacks = LookUpTable::generate_bishop_attacks();
    let bishop_anti_diag_blockers_masks = LookUpTable::generate_bishop_anti_diag_blockers_masks();
    let bishop_diag_blockers_masks = LookUpTable::generate_bishop_diag_blockers_masks();

    let mut bishop_blockers_masks = [0u64; 64];
    for i in 0..64 {
      bishop_blockers_masks[i] = bishop_anti_diag_blockers_masks[i] | bishop_diag_blockers_masks[i];
    }

    LookUpTable {
      pawn_attacks: ByColor::new(LookUpTable::generate_white_pawn_attacks(), LookUpTable::generate_black_pawn_attacks()),
      knight_attacks: LookUpTable::generate_knight_mask(),
      king_attacks: LookUpTable::generate_king_mask(),
      bishop_blockers_masks,
      bishop_magics: bishop_magics_and_attacks.0,
      bishop_attacks: bishop_magics_and_attacks.1,
      rook_blockers_masks: LookUpTable::generate_rook_blockers_masks(),
      rook_magics: rook_magics_and_attacks.0,
      rook_attacks: rook_magics_and_attacks.1,
    }
  }

  fn generate_rook_attacks() -> ([u64; 64], Box<[[u64; 4096]; 64]>) {
    let mut magic_numbers = [0u64; 64];
    let mut attacks = Box::new([[0u64; 4096]; 64]);

    for square in 0..64 {
      let occupancy_combos = LookUpTable::generate_rook_occupancy_combos(&square);
      loop {
        let mut ok = true;
        ...
        for (occupancy, moves) in occupancy_combos {
          ...
          assert!(index < 4096, "Index {} out of range for square {}", index, square);
  ...
  fn generate_bishop_attacks() -> ([u64; 64], Box<[[u64; 512]; 64]>) {
    let mut magic_numbers = [0u64; 64];
    let mut attacks = Box::new([[0u64; 512]; 64]);

    for square in 0..64 {
      let occupancy_combos = LookUpTable::generate_bishop_occupancy_combos(&square);
      loop {
        let mut ok = true;
        let magic: u64 = random_u64_few_bits(); // Increase the chances of finding a magic number
        ...
        for (occupancy, moves) in occupancy_combos.clone() {
          ...
          assert!(index < 512, "Index {} out of range for square {}", index, square);
          if attacks[square as usize][index as usize] == 0 {
            attacks[square as usize][index as usize] = moves;
          } else if attacks[square as usize][index as usize] != moves {
            ok = false;
  ...
  fn generate_rook_occupancy_combos(square: &u8) -> [(u64, u64); 4096] {
    let mut occupancy_combos = [(0, 0); 4096];
    let file = square % 8;
    let rank = square / 8;

    let file_occupancy = LookUpTable::generate_rook_rank_1_occupancy(&rank);
    let rank_occupancy = LookUpTable::generate_rook_rank_1_occupancy(&file);

    let mut index = 0;
    for (mut file_mask, mut file_moves) in file_occupancy {
      file_mask = LookUpTable::rank_to_file(&file_mask);
      file_mask <<= file;

      file_moves = LookUpTable::rank_to_file(&file_moves);
      file_moves <<= file;
  ...
  fn generate_bishop_occupancy_combos(square: &u8) -> HashSet<(u64, u64)> {
    let file = square % 8;
    let rank = square / 8;

    let reference = [0, 1, 2, 3, 4, 5, 6, 7];
    let distance_to_anti_diag: i8 = file as i8 - reference[rank as usize] as i8;
    let anti_diag_occupancy = LookUpTable::generate_bishop_anti_diag_occupancy(&(rank));

    let mut final_anti_diag_occupancy = [(0, 0); 64];
    let anti_diag_blockers_masks = LookUpTable::generate_bishop_anti_diag_blockers_masks()[*square as usize];
    let anti_diag_masks = LookUpTable::generate_bishop_anti_diag_masks()[*square as usize];
    let mut i = 0;
    ...
    let diag_occupancy = LookUpTable::generate_bishop_diag_occupancy(&(7 - rank));

    let mut final_diag_occupancy = [(0, 0); 64];
    let diag_blockers_masks = LookUpTable::generate_bishop_diag_blockers_masks()[*square as usize];
    let diag_masks = LookUpTable::generate_bishop_diag_masks()[*square as usize];
    let mut i = 0;
  ...
  fn generate_rook_rank_1_occupancy(square: &u8) -> [(u64, u64); 64] {
  ...
  fn generate_bishop_anti_diag_occupancy(square: &u8) -> [(u64, u64); 64] {
    let temp_occupancy = LookUpTable::generate_rook_rank_1_occupancy(square);
  ...
  fn generate_bishop_diag_occupancy(square: &u8) -> [(u64, u64); 64] {
    let temp_occupancy = LookUpTable::generate_rook_rank_1_occupancy(square);
    let mut occupancy: [(u64, u64); 64] = [(0, 0); 64];
  ...
  fn generate_rook_blockers_masks() -> [u64; 64] {
  ...
  fn generate_bishop_anti_diag_blockers_masks() -> [u64; 64] {
    let mut masks = [0u64; 64];
  ...
  fn generate_bishop_diag_blockers_masks() -> [u64; 64] {
    let mut masks = [0u64; 64];
  ...
  fn generate_bishop_anti_diag_masks() -> [u64; 64] {
    let mut masks = [0u64; 64];
  ...
  fn generate_bishop_diag_masks() -> [u64; 64] {
    let mut masks = [0u64; 64];
