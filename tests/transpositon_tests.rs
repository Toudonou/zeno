...
mod transposition_tests {
    ...
    fn test_entry_encoding_and_decoding() {
        ...
        let evaluation = Evaluation::MateIn(12);

        let tt_entry = TTEntry::new(position.get_hash(), &best_move, depth, &flag, &evaluation);

        assert_eq!(tt_entry.get_hash(), position.get_hash());
